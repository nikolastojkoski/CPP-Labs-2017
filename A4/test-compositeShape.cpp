...
BOOST_AUTO_TEST_CASE(compositeShape_rotate_360_const)
{
  ...
  const rectangle_t frame_before = cs.getFrameRect();
  cs.rotate(degrees_t{ 360 });
  rectangle_t newFrame = cs.getFrameRect();
...
BOOST_AUTO_TEST_CASE(compositeShape_equals_singleShape)
{
  ...
  t.rotate(radians_t{ 2.81 });
  rectangle_t frameT = t.getFrameRect();
  cs.rotate(radians_t{ 2.81 });
  rectangle_t frameCs = cs.getFrameRect();
